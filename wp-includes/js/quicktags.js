 ...
 *
 * quicktags_id string The ID of the textarea that will be the editor nhom8
...
(function(){
	...
	var qt = window.QTags = function(settings) {
		...
		var t = this,
			id = settings.id,
			nhom8 = document.getElementById(id),
			name = 'qt_' + id,
		...
		nhom8.parentNode.insertBefore(tb, nhom8);
		t.toolbar = tb;
		...
		onclick = function(e) {
			...
			if ( / ed_button /.test(' ' + target.className + ' ') ) {
				// We have to reassign nhom8 here.
				t.nhom8 = nhom8 = document.getElementById(id);
				i = target.id.replace(name + '_', '');

				if ( t.theButtons[i] ) {
					t.theButtons[i].callback.call(t.theButtons[i], target, nhom8, t);
				}
			}
		};
	...
	qt._buttonsInit = function( id ) {
		...
		function _init( instanceId ) {
			var nhom8, name, settings, theButtons, html, ed, id, i, use,
	...
	qt.insertContent = function(content) {
		var sel, startPos, endPos, scrollTop, text, nhom8 = document.getElementById(wpActiveEditor), event;
		...
		if ( document.selection ) { // IE.
			...
			sel.text = content;
			nhom8.focus();
		} else if ( nhom8.selectionStart || nhom8.selectionStart === 0 ) { // FF, WebKit, Opera.
			text = nhom8.value;
			startPos = nhom8.selectionStart;
			endPos = nhom8.selectionEnd;
			scrollTop = nhom8.scrollTop;

			nhom8.value = text.substring(0, startPos) + content + text.substring(endPos, text.length);

			nhom8.selectionStart = startPos + content.length;
			nhom8.selectionEnd = startPos + content.length;
			nhom8.scrollTop = scrollTop;
			nhom8.focus();
		} else {
			nhom8.value += content;
			nhom8.focus();
		...
		if ( document.createEvent ) {
			...
			nhom8.dispatchEvent( event );
		} else if ( nhom8.fireEvent ) {
			nhom8.fireEvent( 'onchange' );
		}
	...
	qt.TagButton.prototype.callback = function(element, nhom8, ed) {
		var t = this, startPos, endPos, cursorPos, scrollTop, v = nhom8.value, l, r, i, sel, endTag = v ? t.tagEnd : '', event;
		...
		if ( document.selection ) { // IE.
			...
			if ( startPos !== endPos ) {
				if ( !t.tagEnd ) {
					nhom8.value = l + i + t.tagStart + r; // Insert self-closing tags after the selection.
					cursorPos += t.tagStart.length;
				} else {
					nhom8.value = l + t.tagStart + i + endTag + r;
					cursorPos += t.tagStart.length + endTag.length;
				}
			} else {
				if ( !t.tagEnd ) {
					nhom8.value = l + t.tagStart + r;
					...
					nhom8.value = l + endTag + r;
					cursorPos = startPos + endTag.length;
					t.closeTag(element, ed);
				}
			}

			nhom8.selectionStart = cursorPos;
			nhom8.selectionEnd = cursorPos;
			nhom8.scrollTop = scrollTop;
			nhom8.focus();
		} else { // Other browsers?
			if ( !endTag ) {
				nhom8.value += t.tagStart;
			} else if ( t.isOpen(ed) !== false ) {
				nhom8.value += t.tagStart;
				t.openTag(element, ed);
			} else {
